...
add_arg('batch_size', int, 512, '训练的批量大小')
add_arg('num_workers', int, 8, '读取数据的线程数量')
add_arg('num_epoch', int, 100, '训练的轮数')
add_arg('num_class', int, 5, '分类的类别数量')
add_arg('learning_rate', float, 0.5, '初始学习率的大小')
add_arg('train_list_path', str, 'ESD/train_list.txt', '训练数据的数据列表路径')
...
add_arg('device_type', str, 'cuda', '设备类型 cuda/cpu')
